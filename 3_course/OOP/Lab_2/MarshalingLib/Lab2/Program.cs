...
namespace Lab2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            myGenericClass.Dispose();

            for (int i = 0; i < myGenericClass.Count; i++)
                Console.WriteLine($"myGeneric {i} = {myGenericClass[i]}");

            Console.WriteLine("\nMarshaling\n");
            MyMarshaling myMarshaling = new(3);
            Console.WriteLine("\nMyLibrary\n");
            MyLibrary.Hi();
            Console.WriteLine(MyLibrary.ComputingStart(4));
        }
    ...
    // ----------------------Lab 2---------------------------------------
    class MyGenericClass<T> : IDisposable
    {
        ...
        {
            if (disposed) throw new ObjectDisposedException("ResourceFreed");
        ...
        public void Dispose()
        {
            // if parameter is set to true, it means that
            // the user of the object initiated the cleanup
            Dispose(true);
            // Cancel the work of the garbage collector for this class
            GC.SuppressFinalize(this);
        }

        protected virtual void Dispose(bool disposing)
        {
            // check that the resources have not been freed yet
            if (!this.disposed)
            {
                // if disposing == true, release all resources
                if (disposing)
                {
    ...
    class MyMarshaling
    {
        ...
        public static extern int Calcul(int CurrentExchangeRate);
        // ----------------------------------------


        public MyMarshaling(int CurrentExchangeRate)
        {
            Console.WriteLine("\n" + Calcul(CurrentExchangeRate));
        }
    }
